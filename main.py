...
class Plugin:
    ...
    async def refresh_devices(self) -> list[dict[str, Any]]:
        return await self.service.refresh_devices()

    async def set_enabled(
        self, enabled: bool, device_id: str | None = None
    ) -> dict[str, Any]:
        return await self.service.set_enabled(enabled, device_id)

    async def set_profile(self, profile_id: str) -> dict[str, Any]:
        return await self.service.set_profile(profile_id)

    async def create_profile(self, name: str) -> dict[str, Any]:
        return await self.service.create_profile(name)
    ...
    async def save_binding(
        ...
        return await self.service.save_binding(profile_id, binding)

    async def delete_binding(
        self, profile_id: str, binding_id: str
    ) -> dict[str, Any]:
        return await self.service.delete_binding(profile_id, binding_id)

    async def start_learning(self) -> None:
        await self.service.start_learning()

    async def stop_learning(self) -> None:
        await self.service.stop_learning()

    async def begin_calibration(self, inputs: list[dict[str, Any]]) -> None:
        await self.service.begin_calibration(inputs)

    async def finish_calibration(
        self, profile_id: str, centers: dict[str, int] | None = None
    ) -> dict[str, Any]:
        return await self.service.finish_calibration(profile_id, centers)
    ...
    async def set_calibration(
        ...
        return await self.service.set_calibration(profile_id, input_value, calibration)
